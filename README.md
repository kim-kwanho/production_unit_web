...
| `/studio` | 한 화면 — OOP Lab + Dashboard 동시 시연 |
...
1. **전체 가동**으로 라인 가동 (에너지 한도 도달 시 비활성).
2. 프리셋 또는 직접 입력 후 **파이프라인 실행**.
3. **+ 유닛 추가** — 타입 선택, 최대 8칸.
...
- Dashboard의 에너지 표시는 **현재 부하(점유)** 개념입니다. 제품 생산 과정에서는 누적되지만, 완제품이 완료되면 해당 제품에 사용한 에너지를 환급(삭제)하여 생산 전 상태로 복귀합니다.
- 효율(도넛)은 **최근 60초 동안의 사용 에너지(누적)와 완제품 수(누적)**로 계산되어, 환급 정책이 있어도 안정적으로 의미를 갖습니다.
